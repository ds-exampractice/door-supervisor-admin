...
const inputCls = 'w-full px-3 py-2.5 rounded-xl bg-[#F5F7FA] text-sm text-[#2C3E50] focus:outline-none focus:ring-2 focus:ring-[#1565C0] border border-transparent focus:border-[#1565C0]'
...
export default function QuestionsPage() {
  ...
  const selectCls = 'px-4 py-2.5 rounded-xl bg-white border border-gray-200 text-sm text-[#2C3E50] focus:outline-none focus:ring-2 focus:ring-[#1565C0] shadow-sm'
    ...
    <div className="p-6 md:p-8 max-w-6xl mx-auto">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50 text-xs text-gray-500 uppercase tracking-wide">
                    ...
                    <th className="text-left px-5 py-3 font-semibold w-28">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {mockQuestions.map(q => (
                    <>
                      ...
                      >
                        <td className="px-5 py-3.5 font-mono text-gray-500 text-xs">{q.Order}</td>
                        ...
                        <td className="px-5 py-3.5" onClick={e => e.stopPropagation()}>
                          <div className="flex gap-2">
                            <button onClick={() => { setMockForm({ ...q }); setEditMock(q); setModalError('') }}
                              className="text-[#1565C0] text-xs font-semibold px-2 py-1 rounded-lg hover:bg-blue-50 transition">Edit</button>
                            <button onClick={() => setShowDeleteMock(q)}
                              className="text-red-500 text-xs font-semibold px-2 py-1 rounded-lg hover:bg-red-50 transition">Delete</button>
                          </div>
                        </td>
                      </tr>
                      {expandedMock === q.id && (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50 text-xs text-gray-500 uppercase tracking-wide">
                    ...
                    <th className="text-left px-5 py-3 font-semibold">Question</th>
                    <th className="text-left px-5 py-3 font-semibold w-28 hidden md:table-cell">Type</th>
                    <th className="text-left px-5 py-3 font-semibold w-20">Answer</th>
                    <th className="text-left px-5 py-3 font-semibold w-28">Actions</th>
                  </tr>
                </thead>
                  ...
                  {trainQuestions.map(q => (
                    <>
                      <tr
                        ...
                        className="border-b border-gray-50 hover:bg-blue-50/30 cursor-pointer transition"
                      >
                        <td className="px-5 py-3.5 text-gray-400 text-xs">{q.sectionNumber}</td>
                        <td className="px-5 py-3.5 text-[#2C3E50]">
                          <div className="flex items-center gap-2">
      ...
      {editMock && (
        <Modal title={editMock.id === '__new__' ? 'Add Mock Question' : `Edit Question ${editMock.id}`} onClose={() => setEditMock(null)} size="lg">
          <div className="space-y-4">
            ...
            {modalError && <p className="text-red-600 text-sm bg-red-50 px-3 py-2 rounded-xl">{modalError}</p>}
      ...
      {showDeleteMock && (
        <Modal title="Delete Question" onClose={() => setShowDeleteMock(null)} size="sm">
          ...
          <div className="flex gap-3">
            <button onClick={() => setShowDeleteMock(null)} className="flex-1 py-2.5 rounded-xl border border-gray-200 text-sm font-semibold text-gray-600 hover:bg-gray-50">Cancel</button>
      ...
      {showDeleteTraining && (
        <Modal title="Delete Question" onClose={() => setShowDeleteTraining(null)} size="sm">
          ...
          <div className="flex gap-3">
            <button onClick={() => setShowDeleteTraining(null)} className="flex-1 py-2.5 rounded-xl border border-gray-200 text-sm font-semibold text-gray-600 hover:bg-gray-50">Cancel</button>
